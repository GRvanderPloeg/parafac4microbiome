...
Note: this vignette reflects a minimum working example for analysing this dataset due to computational limitations in automatic vignette rendering. Hence, we only look at 1-3 components with 5 random initializations each. These settings are not ideal for real datasets. Please refer to the documentation of `assessNumComponents()` for more information.
...
```
The overview plots show that we can reach ~35% explained variation if we take 3 components. The CORCONDIA for those models are ~98.5, which is well above the minimum requirement of 60. Based on this overview, either 2 or 3 components seems fine.
