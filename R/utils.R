...
reshapeData = function(Xlong, subjectMetadata, featureMetadata, timepointMetadata){
  ...
  subjectInfo = subjectInfo %>% dplyr::mutate(1:nrow(subjectInfo))
  ...
  uniqueTimepoints = unique(timepointMetadata)
  timepointInfo = unique(timepointMetadata) %>% dplyr::as_tibble()
  timepointInfo = timepointInfo %>% dplyr::mutate(index = 1:nrow(timepointInfo))
  colnames(timepointInfo) = c("timepoint", "index")
  ...
  for(k in 1:K){
    X[,,k] = df %>%
      dplyr::filter(timepointMetadata == uniqueTimepoints[k]) %>%
